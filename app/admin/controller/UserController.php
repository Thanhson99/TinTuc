...
class UserController extends Controller{
    ...
    public function save(){
        ...
        $validate->addRule('name', 'string', [ 'min' => 5, 'max' => 100 ])
                 ->addRule('username', 'string', [ 'min' => 5, 'max' => 50 ])
                 ->addRule('email', 'email', [])
        ...
        if($task == "edit"){
            ...
            if(!empty($this->request['form']['password'])){
                $validate->addRule('password', 'password', [ 'match' => $this->request['repass'] ]);
            }
